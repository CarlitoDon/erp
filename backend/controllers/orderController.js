...
const createManualOrder = async (req, res) => {
  ...
  if (
    orderChannel === OrderChannel.WHATSAPP_ACQUISITION &&
    !newCustomer?.phone
  ) {
    return res
      .status(400)
      .json({
        message: "Nomor telepon customer baru wajib diisi untuk akuisisi.",
      });
  }
  ...
  if (!Array.isArray(orderItems) || orderItems.length === 0) {
    return res
      .status(400)
      .json({ message: "Minimal satu item produk harus ditambahkan." });
  }
  ...
  for (const item of orderItems) {
    if (
      ...
      return res
        .status(400)
        .json({
          message: `Produk ID tidak valid untuk item: ${JSON.stringify(item)}`,
        });
    }
    if (
      !item.quantity ||
      ...
      return res
        .status(400)
        .json({
          message: `Kuantitas tidak valid untuk produk ID ${item.productId}`,
        });
    }
    if (typeof item.price !== "number" || item.price < 0) {
      ...
      return res
        .status(400)
        .json({
          message: `Harga tidak valid untuk produk ID ${item.productId}`,
        });
    }
  }
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "Nama, telepon, dan alamat pengiriman wajib diisi." });
  }
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (orderChannel === OrderChannel.WHATSAPP_ACQUISITION && newCustomer) {
        ...
        if (existingCustomerByPhone) {
          console.warn(
            `Customer dengan telepon ${newCustomer.phone} sudah ada (ID: ${existingCustomerByPhone.id}). Menggunakan customer yang ada.`
          );
      ...
      // TODO: Tambahkan else if untuk channel Retensi (cari by existingCustomerId)
        ...
        throw new Error(
          "Logika customer untuk channel ini belum diimplementasikan."
        );
      }
      ...
      const newOrder = await tx.order.create({
        ...
        include: {
          // Sertakan data yg ingin dikembalikan
          items: true,
          customer: true,
    ...
    if (error.message.includes("tidak ditemukan")) {
      ...
      // Error unique constraint phone
      res
        .status(409)
        .json({
          message: "Nomor telepon customer sudah terdaftar di akun lain.",
        });
    } else {
      res
        .status(500)
        .json({ message: "Gagal membuat order: " + error.message });
    }
  }
};
